 ...
 */
class ActionCategory extends ActiveRecordMultiLang
{
    ...
    public function getRoute($subdomain)
    {
        if (empty(Yii::$app->request->city)) {
            return ['action/index','pid' => $this->url, 'subdomain' => $subdomain];
        }
        return ['action/index','pid' => $this->url];
    }
