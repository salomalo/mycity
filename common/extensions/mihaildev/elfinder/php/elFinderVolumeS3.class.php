...
//		echo \yii\helpers\BaseVarDumper::dump($dn, 10, true); die('$dn'); // return '/'
		return $dn;
	}
...
//            echo \yii\helpers\BaseVarDumper::dump($path, 10, true); die('$path'); // return 'post/';
	 ...
	 **/
		protected function _joinPath($dir, $name) {
                echo \yii\helpers\BaseVarDumper::dump($dir.DIRECTORY_SEPARATOR.$name, 10, true); die('_joinPath');
	...
	protected function _normpath($path) {
		...
		$tmp =  preg_replace("/\/$/", "", $tmp);
//                echo \yii\helpers\BaseVarDumper::dump($tmp, 10, true); die('$tmp');  // return 'post';
		return $tmp;
	}
...
//		echo \yii\helpers\BaseVarDumper::dump($ret, 10, true); die('$ret');  // return '';
		return $ret;
	}
...
//            echo \yii\helpers\BaseVarDumper::dump($path, 10, true); die('_abspath'); return '/';
...
//            echo \yii\helpers\BaseVarDumper::dump($this->rootName.($path == $this->root ? '' : $this->separator.$this->_relpath($path)), 10, true); die('_path'); return 'S3';
...
//            echo \yii\helpers\BaseVarDumper::dump($path == $parent || strpos($path, $parent.'/') === 0, 10, true); die('_inpath'); 
...
//                 echo \yii\helpers\BaseVarDumper::dump($arr, 10, true); die('metaobj2array'); 
		return $arr;
	}
	...
	protected function _stat($path) {
		...
		//echo \yii\helpers\BaseVarDumper::dump($this->root, 10, true).'-1'; 
		if ($this->root == $path) {
			return $stat;
		}
                //echo \yii\helpers\BaseVarDumper::dump($path, 10, true); die('$obj222');

		$np = $this->_normpath($path);
                //echo \yii\helpers\BaseVarDumper::dump($np, 10, true); die('$np');
		
//		try {
//			$obj = $this->s3->getObject($this->options['bucket'], $path);
//                        
//                       echo \yii\helpers\BaseVarDumper::dump($obj, 10, true); die('$obj');
//			
//		} catch (Exception $e) {
//		
//		}
...
//		}
                ...
                $obj = $this->s3->headObject(['Bucket' =>$this->options['bucket'], 'Key'=> $path]);
                //echo \yii\helpers\BaseVarDumper::dump($obj, 10, true); die('$obj');
		...
		$mime = $obj['ContentType'];
                ...
                //echo \yii\helpers\BaseVarDumper::dump($obj['ContentType'], 10, true); die('$mime');
		...
		try {
			//$files = $this->s3->ListBucket(array('Bucket' => $this->options['bucket'], 'Prefix' => $np, 'Delimiter' => '/'));
			
                        $files = $this->s3->listObjects(['Bucket' => $this->options['bucket']]);
                       
                        //echo \yii\helpers\BaseVarDumper::dump($files, 10, true); die('$files');
		} catch (Exception $e) {
			
		}
			
		if (!is_array($files['Contents'])) {
			$files = array($files);
		}
		
		foreach ($files['Contents'] as $file) {
                     //echo \yii\helpers\BaseVarDumper::dump($file, 10, true).' - '.$np; 
			...
			if ($file['Key'] == $t) {
                ...
                //echo \yii\helpers\BaseVarDumper::dump($stat, 10, true); die('$stat');
		return $stat;
	}
	...
	protected function _subdirs($path) {
		...
		//echo \yii\helpers\BaseVarDumper::dump($path, 10, true); die('$stat');
		if ($stat['mime'] == 'directory') {
		 $files = $this->_scandir($path);
                 //echo \yii\helpers\BaseVarDumper::dump($files, 10, true); die('$files');
		 foreach ($files as $file) {
			$fstat = $this->_stat($file);
                        //echo \yii\helpers\BaseVarDumper::dump($fstat, 10, true); 
	...
	protected function _scandir($path) {
		...
		$files = $this->s3->listObjects(['Bucket' => $this->options['bucket']]);
                //$this->s3->listObjects()
                 ...
                 //echo \yii\helpers\BaseVarDumper::dump($files, 10, true); die();
		...
		foreach ($files['Contents'] as $file) {
			if (preg_match("|^" . $s3path . "[^/]*/?$|", $file['Key'])) {
				...
				//$finalfiles[] = 'https://s3-eu-west-1.amazonaws.com/files1q/'.$fname;
				$finalfiles[] = $fname;
			}
		}
		
		sort($finalfiles);
                //echo \yii\helpers\BaseVarDumper::dump($finalfiles, 10, true); die('$finalfiles');
		return $finalfiles;
	}
	...
	protected function _mkdir($path, $name) {
		...
		try {
			$obj = $this->s3->putObject(array('Bucket' => $this->options['bucket'], 'Key' => $newkey , 'ContentLength' => 0, 'Data' => ''));
		} catch (Exception $e) {
...
//	protected function _save($fp, $dir, $name, $mime, $w, $h) {
	protected function _save($fp, $dir, $name, $mime) {
		return false;
	}
