...
namespace frontend\extensions\Rss;
...
use frontend\extensions\Rss\Feed;

/**
 * Description of RssView
 *
 * @author dima
 */
    ...
    public function renderItem($model)
    {
        ...
        foreach ($this->items as $element => $value) {
            if ($element === 'enclosure') {
                $result = $value($model);
                if (is_array($result) and isset($result['url'], $result['length'], $result['type'])) {
                    $this->getFeed()->addItemEnclosure($result['url'], $result['length'], $result['type']);
                }
                continue;
            }
            if (is_string($value)) {
