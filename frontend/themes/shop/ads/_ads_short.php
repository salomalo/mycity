...
// если обьявления предприятия, то берем его инфу( чтоб уменьшить число запросов к бд
if (isset($business)){
    $phone = $model ? isset($business->phone) ? $business->phone : null : null;
    ...
    if ($business->city && (($city && ($business->idCity !== $city->id)) || !$city)) {
        $businessUrl = "http://{$business->city->subdomain}.{$front}{$businessUrl}";
    }
} else {
...
<div itemscope="itemscope" itemtype="http://schema.org/Product" class="
    ...
    <div class="b-product-gallery__content">
        ...
        <div class="b-product-gallery__holder">
            ...
            <a href="<?= Url::to(['/ads/view', 'alias' => "{$model->_id}-{$model->url}"]) ?>" itemprop="url"
