...
$gallery = Gallery::find()
    ->select('id')
    ->where(['pid' => $model->id, 'type' => File::TYPE_AFISHA])
    ->orderBy(['id' => SORT_ASC])
    ->asArray()->one();

if ($model->image) {
    $previews[] = Yii::$app->files->getUrl($model, 'image', 100);
    $images[] = Yii::$app->files->getUrl($model, 'image', 900);
}
...
if ($gallery) {
    ...
    // добавляем в галлерею картинку

    //Получаем url изображений и миниатюр
    foreach ($files as $file) {
        $previews[] = Yii::$app->files->getUrl(new Gallery(), 'attachments', 100, $file->name, "afisha/{$gallery['id']}");
        $images[] = Yii::$app->files->getUrl(new Gallery(), 'attachments', 900, $file->name, "afisha/{$gallery['id']}");
    }
}
?>
