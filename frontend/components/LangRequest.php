...
class LangRequest extends Request
{
    ...
    {
        if (!$this->flag) {
            empty(Yii::$app->params['city']) ?: ($this->_city = Yii::$app->params['city']);

            if (empty($this->_city)) {
                $notCity = ['www', 'backend', 'office'];
                $sub = explode('.', $this->serverName);
                if ((count($sub) > 2) and !in_array($sub[0], $notCity)) {
                    $this->_city = City::find()->where(['subdomain' => $sub[0]])->one();
                }
            }
            $this->flag = true;
        }
