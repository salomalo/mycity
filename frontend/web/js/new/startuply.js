...
Startuply = {
    ...
    cartCheckoutInit: function () {
        //запрещаем переход по ссылкам меню корзины
        // $(".cart-checkout-navigation-list a").click(function (e) {
        //     return false;
        // });
        ...
        {
            valid = true;
            if (($('#shoppingcartform-idcity').val() == "") || ($('#shoppingcartform-idcity').val() == null)) {
                alert ( "Необходимо заполнить «Город»." );
                valid = false;
            }else if ($('#shoppingcartform-delivery').val() == "") {
                alert ( "Необходимо заполнить «Способ доставки»." );
                valid = false;
            }else if ($('#shoppingcartform-office').val() == "") {
                alert ( "Необходимо заполнить «Отделение»." );
                valid = false;
            // }else if ($('#shoppingcartform-address').val() == ""){
            //     alert ( "Необходимо заполнить «Адрес»." );
            //     valid = false;
            }else if ($('#shoppingcartform-phone').val() == ""){
        ...
        function validateSteps($index) {
            switch ($index) {
                case 0:
                    var rowCount = $('#table-cart-list tr').length;
                    if (!rowCount){
        ...
        if ( $('.cart-checkout-navigation-list').length ) {
            ...
            $navControls.on('click.startuply', '.btn', function (event) {
                ...
                //если нажали на слудующий шаг
                if ($(this).hasClass('next')) {
                    $isValid = validateSteps($activeLi.index());

                    if ($isValid){
                        $activeLi.next().find('.cart-checkout-navigation-list-item-link').trigger('click');
                    }
                    //если нажали на предыдущий шаг
