jQuery(document).ready(function($) {
    ...
    if (listingDetailMenu.length) {
         ...
         * Listing menu scroll
         */
        listingDetailMenu.find('a').on('click', function(e) {
            e.preventDefault();

            var href = $(this).attr('href');

            if (href.charAt(0) === '#') {
                $.scrollTo(href, 1200, {axis: 'y', offset: -160});
            } else {
        ...
        if (listingDetailMenu.length !== 0) {
            ...
            listingDetailMenu.affix({offset: {top: height}});
        }

        /**
         * Scroll spy
         */
        $('body').scrollspy({target: '.listing-detail-menu', offset: 33});
    }
    ...
    if (listingDetailPrice.length) {
        ...
        listingDetailPrice.affix({offset: {top: 34}});
    }

    if ($(document).outerHeight(true) > 800) {
        // Sticky header
        $('.header-sticky .header-wrapper').affix({offset: {top: 33}});
    }
