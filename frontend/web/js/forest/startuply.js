...
$(document).on('ready', function () {
    jQuery(function ($) {
        $("#lead-phone").mask("+38 (999) 999-99-99");
        $("#bingc-passive-get-phone-form-input").mask("+38 (999) 999-99-99");
    });
    function isCorectNumber(phone){
        return phone.toString().length >= 10;
    }

    function callbackToPhone(phone) {
        $.ajax({
    ...
    //при нажатии на звонок
    $("#bingc-phone-button").click(function (e) {
        $("#bingc-passive").removeClass("bingc-passive-closed");
        $("#bingc-phone-button").addClass("bingc-hide");
    });

    //событие при закрытии окна звонка
    $("#bingc-passive-close-button").click(function (e) {
        $("#bingc-passive").addClass("bingc-passive-closed");
        $("#bingc-phone-button").removeClass("bingc-hide");

        $("#main-content").removeClass("content-hide");
        $("#second-content").addClass("content-hide");
    });

    //событие при отправки телефона
    $("#bingc-passive-phone-form-button").click(function (e) {
        var phoneInput = $('#bingc-passive-get-phone-form-input');
        var phone = phoneInput.val().toString().replace(/\D/g, "");

        if (isCorectNumber(phone)) {
            phoneInput.removeClass("warning");

            callbackToPhone(phone);

            $("#second-content").removeClass("content-hide");
            $("#main-content").addClass("content-hide");
            phoneInput.val('');
            ga('send','event','request-for-perezvoni','send');
        } else {
    ...
    $("#bingc-phone-button").mouseover(function () {
        $("#bingc-phone-button-tooltip").addClass("bingc-phone-button-tooltip-show-first bingc-phone-button-tooltip-show-second");
    });

    $("#bingc-phone-button").mouseout(function () {
        $("#bingc-phone-button-tooltip").removeClass("bingc-phone-button-tooltip-show-first bingc-phone-button-tooltip-show-second");
    });

    //крутая анимация
    var text = $("#bingc-phone-button-icon-text"), icon = $("#bingc-phone-button-icon-icon");
    var index = 1;
    ...
    function callback() {
        if (index % 2 == 1){
            text.removeClass("bingc-phone-button-icon-show");
            icon.attr('class', 'bingc-phone-button-icon-icon bingc-phone-button-icon-show');

            interval = 8000;
        } else {
            icon.attr('class', 'bingc-phone-button-icon-icon');
            text.addClass("bingc-phone-button-icon-show");
...
$( "#btn-for-login-form" ).click(function(e) {
    e.preventDefault();
...
$( "#btn-for-consult-form" ).click(function(e) {
    e.preventDefault();
...
$( "#go-to-consultation" ).click(function(e) {
    $( "#form-consult" ).show();
    $( "#form-register" ).hide();
});
